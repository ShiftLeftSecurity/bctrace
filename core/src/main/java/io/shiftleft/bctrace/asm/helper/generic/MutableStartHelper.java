 ...
 */
public class MutableStartHelper extends Helper {
  ...
  /**
   * Depending on the {@link MutableStartListener} listeners that apply to this method,  this helper
  ...
  private boolean addMutableTraceStart(ClassNode cn, MethodNode mn,
      ArrayList<Integer> hooksToUse) {
    ArrayList<Integer> listenersToUse = getListenersOfType(hooksToUse, MutableStartListener.class);
    ...
    for (int i = 0; i < listenersToUse.size(); i++) {
      ...
      MutableStartListener listener = (MutableStartListener) bctrace.getHooks()[index]
          .getListener();
